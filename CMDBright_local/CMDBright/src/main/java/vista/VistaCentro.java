...
@Component("vistaCentro")
@Scope("session")
public class VistaCentro implements Serializable {
	...
	private Centro datosForm;
	private Centro seleccionado;
	...
	public List<Centro> getCentrosFiltrados() {
		return centrosFiltrados;
	}
	public void setCentrosFiltrados(List<Centro> centrosFiltrados) {
		this.centrosFiltrados = centrosFiltrados;
	}
	public SCentro getS() {
		return s;
	}
	public void setS(SCentro s) {
		this.s = s;
	}
	public Centro getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Centro datosForm) {
		this.datosForm = datosForm;
	}
	public Centro getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(Centro seleccionado) {
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaCentro");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Centro(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
		headerPanel = "--- AÑADIR ---";
		seleccionado = new Centro();
		seleccionado = null;
	}
	public void edit() {
		bloquearPanel();
		...
		seleccionado = new Centro(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO centro guardado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNombre() + ": ", "Ya existe un centro con ese nombre"));
			datosForm = new Centro(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Centro modificado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNombre() + ": ", "Ya existe un centro con ese nombre"));
			datosForm = new Centro(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Centro BORRADO: " + datosForm.getNombre()));		
