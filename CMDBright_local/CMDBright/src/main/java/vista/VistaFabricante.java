...
@Component("vistaFabricante")
@Scope("session")
public class VistaFabricante implements Serializable {
	...
	@Autowired
	private SFabricante s;
	
	private Fabricante datosForm;
	private Fabricante seleccionado;
	...
	public SFabricante getS() {
		return s;
	}
	public void setS(SFabricante s) {
		this.s = s;
	}
	public Fabricante getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Fabricante datosForm) {
		this.datosForm = datosForm;
	}
	public Fabricante getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(Fabricante seleccionado) {
	...
	public void init() {
		...
		fabricantes = s.findAll();
		total = fabricantes.size();
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaFabricante");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Fabricante(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
		headerPanel = "--- AÑADIR ---";
		seleccionado = new Fabricante();
		seleccionado = null;
	}
	public void edit() {
		bloquearPanel();
		...
		seleccionado = new Fabricante(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO fabricante guardado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNombre() + ": ", "Ya existe un fabricante con ese nombre"));
			datosForm = new Fabricante(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Fabricante modificado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNombre() + ": ", "Ya existe un fabricante con ese nombre"));
			datosForm = new Fabricante(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Fabricante BORRADO: " + datosForm.getNombre()));		
