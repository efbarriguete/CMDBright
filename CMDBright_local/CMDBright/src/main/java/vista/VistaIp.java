...
@Component("vistaIp")
@Scope("session")
public class VistaIp implements Serializable {
	private static final long serialVersionUID = 3895369471017824663L;

	private List<Ip> ips;
	private List<Ip> ipsFiltrados;
	...
	private Ip datosForm;
	private Ip seleccionado;
	...
	public List<Ip> getIpsFiltrados() {
		return ipsFiltrados;
	}
	public void setIpsFiltrados(List<Ip> ipsFiltrados) {
		this.ipsFiltrados = ipsFiltrados;
	}
	public SIp getS() {
		return s;
	}
	public void setS(SIp s) {
		this.s = s;
	}
	public Ip getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Ip datosForm) {
		this.datosForm = datosForm;
	}
	public Ip getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(Ip seleccionado) {
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaIp");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Ip(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
	...
	public void edit() {
		...
		seleccionado = new Ip(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			datosForm.setUsed(false);
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO ip guardado: " + datosForm.getIp()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getIp() + ": ", "Ya existe un ip con ese nombre"));
			datosForm = new Ip(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Ip modificado: " + datosForm.getIp()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getIp() + ": ", "Ya existe un ip con ese nombre"));
			datosForm = new Ip(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Ip BORRADO: " + datosForm.getIp()));		
