...
@Component("vistaModel")
@Scope("session")
public class VistaModel implements Serializable {
	private static final long serialVersionUID = 3895369471017824663L;

	private List<Model> models;
	private List<Model> modelsFiltrados;
	private List<Fabricante> fabricantes;
	private List<TipoModel> tipoModelos;
	@Autowired
	private STipoModel sTM;
	@Autowired
	private SFabricante sFab;
	@Autowired
	private SModel s;
	
	private Model datosForm;
	private Model seleccionado;
	...
	public List<Model> getModelsFiltrados() {
		return modelsFiltrados;
	}
	public void setModelsFiltrados(List<Model> modelsFiltrados) {
		this.modelsFiltrados = modelsFiltrados;
	}
	public List<TipoModel> getTipoModelos() {
		tipoModelos = sTM.findAll();
		return tipoModelos;
	}
	public List<Fabricante> getFabricantes() {
		fabricantes = sFab.findAll();
		return fabricantes;
	}
	public SModel getS() {
		return s;
	}
	public void setS(SModel s) {
		this.s = s;
	}
	public void setsFab(SFabricante sFab) {
		this.sFab = sFab;
	}
	public void setsTM(STipoModel sTM) {
		this.sTM = sTM;
	}
	public Model getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Model datosForm) {
		this.datosForm = datosForm;
	}
	public Model getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(Model seleccionado) {
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaModel");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Model(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
		headerPanel = "--- AÑADIR ---";
		seleccionado = new Model();
		seleccionado = null;
	}
	public void edit() {
		bloquearPanel();
		...
		seleccionado = new Model(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO model guardado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNombre() + ": ", "Ya existe un model con ese nombre"));
			datosForm = new Model(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Model modificado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNombre() + ": ", "Ya existe un model con ese nombre"));
			datosForm = new Model(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Model BORRADO: " + datosForm.getNombre()));		
	...
	public String cancelar() {
		init(); //inicio la vista
		return null; //¿puse esto porque me saltaba algún error??? por uqé usar public String en vez public void????
	}	
}