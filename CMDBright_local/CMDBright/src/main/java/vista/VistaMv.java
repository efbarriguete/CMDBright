...
@Component("vistaMv")
@Scope("session")
public class VistaMv implements Serializable {
	private static final long serialVersionUID = 3895369471017824663L;

	private List<Mv> mvs;
	private List<Mv> mvsFiltrados;
	
	@Autowired
	private SMv s;
	
	private Mv datosForm;
	private Mv seleccionado;
	...
	public List<Mv> getMvsFiltrados() {
		return mvsFiltrados;
	}
	public void setMvsFiltrados(List<Mv> mvsFiltrados) {
		this.mvsFiltrados = mvsFiltrados;
	}
	public SMv getS() {
		return s;
	}
	public void setS(SMv s) {
		this.s = s;
	}
	public Mv getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Mv datosForm) {
		this.datosForm = datosForm;
	}
	public Mv getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(Mv seleccionado) {
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaMv");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Mv(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
	...
	public void edit() {
		...
		seleccionado = new Mv(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO mv guardado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNombre() + ": ", "Ya existe un mv con ese nombre"));
			datosForm = new Mv(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Mv modificado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNombre() + ": ", "Ya existe un mv con ese nombre"));
			datosForm = new Mv(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Mv BORRADO: " + datosForm.getNombre()));		
