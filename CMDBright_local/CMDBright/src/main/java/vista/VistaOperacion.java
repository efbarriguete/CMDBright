...
@Scope("session")
public class VistaOperacion implements Serializable {
	private static final long serialVersionUID = 3895369471017824663L;

	private List<Operacion> operacions;
	private List<Operacion> operacionsFiltrados;
	private List<Proveedor> proveedores;
	private List<TipoOperacion> tipoOperaciones;
	@Autowired
	private STipoOperacion sO;
	@Autowired
	private SProveedor sP;
	@Autowired
	private SOperacion s;
	
	private Operacion datosForm;
	...
	public List<Operacion> getOperacionsFiltrados() {
		return operacionsFiltrados;
	}
	public void setOperacionsFiltrados(List<Operacion> operacionsFiltrados) {
		this.operacionsFiltrados = operacionsFiltrados;
	}
	public List<Proveedor> getProveedores() {
		proveedores = sP.findAll();
		return proveedores;
	}
	public List<TipoOperacion> getTipoOperaciones() {
		tipoOperaciones = sO.findAll();
		return tipoOperaciones;
	}
	public SOperacion getS() {
		return s;
	}
	//3 setters para el autowired:
	public void setS(SOperacion s) {
		this.s = s;
	}
	public void setsO(STipoOperacion sO) {
		this.sO = sO;
	}
	public void setsP(SProveedor sP) {
		this.sP = sP;
	}
	public Operacion getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Operacion datosForm) {
		this.datosForm = datosForm;
	}
	public Operacion getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(Operacion seleccionado) {
	...
	public void init() {
		...
		operacions = s.findAll();
		total = operacions.size();
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaOperacion");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Operacion(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
	...
	public void edit() {
		...
		seleccionado = new Operacion(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO operacion guardado: " + datosForm.getNumOperacion()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNumOperacion() + ": ", "Ya existe un operacion con ese nombre"));
			datosForm = new Operacion(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Operacion modificado: " + datosForm.getNumOperacion()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNumOperacion() + ": ", "Ya existe un operacion con ese nombre"));
			datosForm = new Operacion(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Operacion BORRADO: " + datosForm.getNumOperacion()));		
