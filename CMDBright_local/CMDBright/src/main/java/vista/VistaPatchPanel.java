...
@Component("vistaPatchPanel")
@Scope("session")
public class VistaPatchPanel implements Serializable {
	private static final long serialVersionUID = 3895369471017824663L;

	private List<PatchPanel> patchPanels;
	private List<PatchPanel> patchPanelsFiltrados;
	private List<Rack> racks;
	@Autowired
	private SRack sX;
	
	@Autowired
	private SPatchPanel s;
	
	private PatchPanel datosForm;
	private PatchPanel seleccionado;
	...
	public List<PatchPanel> getPatchPanelsFiltrados() {
		return patchPanelsFiltrados;
	}
	public void setPatchPanelsFiltrados(List<PatchPanel> patchPanelsFiltrados) {
		this.patchPanelsFiltrados = patchPanelsFiltrados;
	}
	public SPatchPanel getS() {
		return s;
	}
	public void setS(SPatchPanel s) {
		this.s = s;
	}
	public PatchPanel getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(PatchPanel datosForm) {
		this.datosForm = datosForm;
	}
	public PatchPanel getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(PatchPanel seleccionado) {
	...
	public void init() {
		...
		patchPanels = s.findAll();
		total = patchPanels.size();
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaPatchPanel");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new PatchPanel(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
		headerPanel = "--- AÑADIR ---";
		seleccionado = new PatchPanel();
		seleccionado = null;
	}
	public void edit() {
		bloquearPanel();
		...
		seleccionado = new PatchPanel(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO patchPanel guardado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNombre() + ": ", "Ya existe un patchPanel con ese nombre"));
			datosForm = new PatchPanel(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "PatchPanel modificado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNombre() + ": ", "Ya existe un patchPanel con ese nombre"));
			datosForm = new PatchPanel(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "PatchPanel BORRADO: " + datosForm.getNombre()));		
