...
@Component("vistaPlanta")
@Scope("session")
public class VistaPlanta implements Serializable {
	private static final long serialVersionUID = 3895369471017824663L;

	private List<Planta> plantas;
	private List<Planta> plantasFiltrados;
	...
	private Planta datosForm;
	private Planta seleccionado;
	...
	public List<Planta> getPlantasFiltrados() {
		return plantasFiltrados;
	}
	public void setPlantasFiltrados(List<Planta> plantasFiltrados) {
		this.plantasFiltrados = plantasFiltrados;
	}
	public SPlanta getS() {
		return s;
	}
	public void setS(SPlanta s) {
		this.s = s;
	}
	public Planta getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Planta datosForm) {
		this.datosForm = datosForm;
	}
	public Planta getSeleccionado() {
		return seleccionado;
	}
	public void setSeleccionado(Planta seleccionado) {
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaPlanta");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Planta(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
		headerPanel = "--- AÑADIR ---";
		seleccionado = new Planta();
		seleccionado = null;
	}
	public void edit() {
		bloquearPanel();
		...
		seleccionado = new Planta(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO planta guardado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNombre() + ": ", "Ya existe un planta con ese nombre"));
			datosForm = new Planta(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Planta modificado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNombre() + ": ", "Ya existe un planta con ese nombre"));
			datosForm = new Planta(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Planta BORRADO: " + datosForm.getNombre()));		
