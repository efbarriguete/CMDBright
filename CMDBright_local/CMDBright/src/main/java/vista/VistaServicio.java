...
@Scope("session")
public class VistaServicio implements Serializable {
	...
	public List<Servicio> getServiciosFiltrados() {
		return serviciosFiltrados;
	}
	public void setServiciosFiltrados(List<Servicio> serviciosFiltrados) {
		this.serviciosFiltrados = serviciosFiltrados;
	}
	public SServicio getS() {
		return s;
	}
	public void setS(SServicio s) {
		this.s = s;
	}
	public Servicio getDatosForm() {
		return datosForm;
	}
	public void setDatosForm(Servicio datosForm) {
	...
	public void resetBean() {
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("vistaServicio");
    }
	...
	public void add() {
		...
		vaciaValoresForm();
		datosForm = new Servicio(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		mostrarSavBtn = true;
		mostrarModBtn = false;
		mostrarDelBtn = false;
		estadoPanel = true;
	...
	public void edit() {
		...
		seleccionado = new Servicio(); //para que no se me borre una línea en la tabla de datos, antes de poner el objeto a null, lo instancio.
		seleccionado = null;
	}
	...
	public String grabarDatosNew() {
		try {
			s.create(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "NUEVO servicio guardado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible agregar: " + datosForm.getNombre() + ": ", "Ya existe un servicio con ese nombre"));
			datosForm = new Servicio(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
	...
	public String grabarDatosMod() {
		try {
			s.update(datosForm);
			FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_INFO, "Info:", "Servicio modificado: " + datosForm.getNombre()));		
			init(); //inicio la vista
		} catch (DataIntegrityViolationException ex) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Imposible actualizar: " + datosForm.getNombre() + ": ", "Ya existe un servicio con ese nombre"));
			datosForm = new Servicio(); //Instancio un nuevo modelo, ya que de momento m apunta a null.
		}
			return null; //¿puse esto porque me saltaba algún error???
	}
	public String borrar() {
		...
		s.delete(datosForm);
		FacesContext.getCurrentInstance().addMessage("mensajes", new FacesMessage(FacesMessage.SEVERITY_WARN, "Atención:", "Servicio BORRADO: " + datosForm.getNombre()));		
